...
class BluebirdCollisionAvoidancePointSequencePlannerNode():
    ...
    def work(self):
        ...
        # initialize node
        rospy.init_node('bluebird_collision_avoidance_point_sequence_planner_node')
        ...
        topic = rospy.get_param('other_pos_topic', default='msf_core/odometry')
        ...
        while not self._got_other_initial_pos_flag and not rospy.is_shutdown():
            print("The planner is waiting for the partners initial position.")
            rate.sleep()
        ...
        while not rospy.is_shutdown():
            ...
            if numpy.linalg.norm(self._pos-self._waypoint) < 0.2:
                print("\nNew waypoint generated: " + str(waypoint_counter) + "\n")
                print("\nat time: " + str(rospy.get_time()) + "\n")
                waypoint_counter += 1
                self._generate_waypoint()
...
if __name__ == '__main__':
    node = BluebirdCollisionAvoidancePointSequencePlannerNode()
    node.work()
